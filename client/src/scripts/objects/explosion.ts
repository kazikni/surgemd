...
export class Explosion extends GameObject{
    ...
    explode(){
        ...
        if(this.def.cam_shake){
            if(v2.distance(this.position,this.scene.camera.position)<=(this.def.cam_shake.distance??40))this.scene.camera.shake(this.def.cam_shake.intensity*0.2,this.def.cam_shake.duration)
        }
    }
