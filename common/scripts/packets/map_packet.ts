...
export class MapPacket extends Packet{
    ID=PacketType.Map
    Name="map"
    map:MapConfig={minimap_enabled:false,biome:NormalBiome,terrain:[],size:v2.zero(),seed:0,objects:[],regions:[]}
    constructor(){
        super()
    }
    encode(stream: Stream): void {
        stream.write_boolean_group(this.map.minimap_enabled)
        ...
        if(this.map.minimap_enabled){
            stream.write_array(this.map.objects,(i)=>{
        ...
        stream.write_uint16(this.map.size.x)
    ...
    decode(stream: Stream): void {
        ...
        this.map.minimap_enabled=minimap
