import { Angle, astar_path2d, BTAction, BTCondition, BTNode, BTSelector, BTSequence, BTState, Numeric, random, Stream, v2, v2m, Vec2 } from "common/engine/core.ts";
...
export class CombatNode implements BTNode<BotExecutionContext> {
    tick(ctx: BotExecutionContext): BTState {
        ...
        if(dist>1.5&&dist<20&&ctx.ai.is_aim_aligned(ctx.target_pos)&&!ctx.human.input.reload){
...
export class ADVHumanAILegacy extends BotAi{
    controller={
